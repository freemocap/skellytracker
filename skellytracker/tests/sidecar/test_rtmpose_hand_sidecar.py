"""Tests for the RTMPose hand sidecar (M4b): loads the real rtmpose-hand.yaml
...
class TestRTMPoseHandSidecarValidation:
    def test_loads_and_validates(self):
        sidecar = load_sidecar(_HAND_DIR / "rtmpose-hand.yaml")
        assert sidecar.model_id == "rtmpose-hand"
    ...
    def test_tracked_points_count(self):
        sidecar = load_sidecar(_HAND_DIR / "rtmpose-hand.yaml")
        assert len(sidecar.pose.tracked_points) == 21
        assert len(set(sidecar.pose.tracked_points)) == 21
        assert sidecar.pose.tracked_points[0] == "root"
        assert sidecar.pose.tracked_points[-1] == "pinky_finger4"

    def test_canonical_mapping_renames(self):
        sidecar = load_sidecar(_HAND_DIR / "rtmpose-hand.yaml")
        mapping = sidecar.pose.canonical_mapping
        assert mapping["wrist"] == "root"
        assert mapping["thumb_tip"] == "thumb4"
...
class TestRTMPoseHandParityAgainstGoldenFixture:
    def test_detect_matches_golden_fixture_by_name(self, model_name, test_image):
        ...
        golden_path = (
            _GOLDEN_DIR / f"rtmpose_hand_{model_name.replace('-', '_')}_detect.json"
        )
        ...
        config = OnnxSessionConfig(
            batch_size=1, models=[RTMPoseHandDetector.model_spec(model_name)]
        )
        session = OnnxSession.create(config)
        try:
            detector = RTMPoseHandDetector.create(
                RTMPoseHandDetectorConfig(model_name=model_name), session
            )
...
class TestRTMPoseHandModelSpecVerification:
    def test_model_spec_performs_no_network_io(self):
        ...
        with patch(
            ...
            for model_name in _MODEL_NAMES:
                RTMPoseHandDetector.model_spec(model_name)
    ...
    def test_model_spec_populates_expected_filename_and_sha256(self):
        ...
        for model_name in _MODEL_NAMES:
            spec = RTMPoseHandDetector.model_spec(model_name)
            artifact = (
    ...
    def test_resolve_model_path_verifies_matching_sha256(self, tmp_path):
        ...
        spec = RTMPoseHandDetector.model_spec("rtmpose-m_256x256")
    ...
    def test_resolve_model_path_rejects_mismatched_sha256(self, tmp_path):
        ...
        spec = RTMPoseHandDetector.model_spec("rtmpose-m_256x256")
        body = b"fake-zip-bytes-for-hand"
