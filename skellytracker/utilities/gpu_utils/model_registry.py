...
def resolve_model_paths_parallel(
    ...
    Each source is downloaded (if needed) concurrently.  Failures are
    reported per-source — a single failing download does not abort the
    others.

    Returns a dict mapping source list index → resolved ``Path``.
    Sources whose resolution failed are absent from the dict.
    """
    if not sources:
        return {}
