"""RTMPose body keypoint detector (17 keypoints: COCO body).
...
class RTMPoseBodyDetector(KeypointDetector):
    """RTMPose body SIMCC detector — 17 keypoints (COCO body)."""
    ...
    def create(
        ...
        if not isinstance(config, RTMPoseBodyDetectorConfig):
            raise TypeError(
                f"Expected RTMPoseBodyDetectorConfig, got {type(config).__name__}"
            )
    ...
    def model_spec(cls, model_name: str = "rtmpose-m_256x192") -> OnnxModelSpec:
        if model_name not in _SIDECAR.sizes:
            raise ValueError(
                f"Unknown RTMPose body model {model_name!r}. Available: {list(_SIDECAR.sizes)}"
            )
...
RTMPOSE_BODY_MODEL_SPECS: dict[str, OnnxModelSpec] = {
    name: RTMPoseBodyDetector.model_spec(name) for name in _SIDECAR.sizes
}
