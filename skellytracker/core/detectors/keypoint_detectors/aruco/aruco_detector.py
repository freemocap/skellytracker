...
class ArucoDetector(KeypointDetector):
    ...
    def preprocess(self, image: NDArray[np.uint8]) -> tuple[NDArray[np.uint8], EmptyMetadata]:
        """Convert image to greyscale for ArUco detection."""
    ...
    def postprocess(self, raw: Any, metadata: EmptyMetadata) -> Keypoints:
        """Build Keypoints from raw detectMarkers output.

        raw is a 3-tuple (detected_corners, detected_ids, rejected) as returned
        by cv2.aruco.ArucoDetector.detectMarkers(). The rejected element is
        ignored.
        """
        detected_corners, detected_ids, _ = raw
